...
module VM
  ...
  context "VM.matrix_layout={{layout.id}}" do
    it "stores matrix entries in memory {{layout.id}} style" do
      VM.matrix_layout = VM::MatrixLayout::{{layout.id}}
    end

    describe "Mat{{m}}x{{n}}({{scalar.id}})" do
      x = typed_mat.new(
      {% if layout == :RowMajor %}
        {% for i in 1..m %}{% for j in 1..n %} {{i*10 + j}}, {% end %}{% end %}
      {% else %}
        {% for j in 1..n %}{% for i in 1..m %} {{i*10 + j}}, {% end %}{% end %}
      {% end %}
      )

      context "using example matrix for component identification: #{x}" do
    ...
    end # context (RowMajor/ColumnMajor)
    {% end %}
